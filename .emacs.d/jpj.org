...
#+RESULTS:
| er/add-web-mode-expansions | er/add-html-mode-expansions | web-mode-init-prettier-hook | add-node-modules-path | copilot-mode | prettier-js-mode | web-mode-init-hook | web-mode-init-emmet-hook |
...
(defun jpj/add-shadcn-ui-component (component)
  ...
  (let* ((root-dir (find-root-dir-with "package.json"))
         (command (format "bunx --bun shadcn-ui@latest add %s" component)))
    (if root-dir
...
(use-package gptel
  ...
  :straight (:host github :repo "suzuki/gptel" :branch "add-gpt-4-turbo")
  :config
  (require 'gptel-curl)
  ...
  ;; (setq-default gptel-model "claude-3-opus-20240229")
  ...
  (defun gpt-screen ()
    ...
    (let ((content "*** Here is some code I am working on:\n\n"))
      ...
      (gptel-select-api-key my/gptel-api-key-label)
      (gptel (generate-new-buffer-name "*gpt screen*") nil content t))
    (auto-fill-mode 0)
    (visual-line-mode t))

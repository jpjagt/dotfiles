...
(defun crpath ()
  "Copy the current buffer's path relative to projectile project root."
  (interactive)
  (if (and (featurep 'projectile) (projectile-project-p))
      (let* ((full-path (buffer-file-name))
             (project-root (projectile-project-root))
             (rel-path (file-relative-name full-path project-root))
             )
        (kill-new rel-path)
        (message "Copied rel path '%s' to the clipboard." rel-path))
    (message "Not in a projectile project")))

(defun crmod ()
  "Copy the current buffer's path relative to projectile project root as a module name."
  (interactive)
  (if (and (featurep 'projectile) (projectile-project-p))
      (let* ((full-path (buffer-file-name))
             (project-root (projectile-project-root))
             (rel-path (file-relative-name full-path project-root))
             (module-name (replace-regexp-in-string "/" "." (file-name-sans-extension rel-path))))
        (kill-new module-name)
        (message "Copied module name '%s' to the clipboard." module-name))
    (message "Not in a projectile project")))

(defun copy-rel-mpc-path ()
  "Copy a Markdown link with filename and project-relative path: [filename.ext](path/to/filename.ext)"
  (interactive)
  (if (and (featurep 'projectile) (projectile-project-p))
      (let* ((full-path (buffer-file-name))
             (project-root (projectile-project-root))
             ...
             (markdown-link (format "[%s](mdc:%s)" filename rel-path)))
        (kill-new markdown-link)
        (message "Copied Markdown link '%s' to the clipboard." markdown-link))
    (message "Not in a projectile project")))
...
less cumbersome keybind than C-c ^:

#+BEGIN_SRC emacs-lisp
(setq smerge-command-prefix (kbd "s-6"))
#+END_SRC
...
(defun jpj/projectile-open-root-in-finder ()
  (interactive)
  (shell-command (format "open %s" (projectile-acquire-root))))

(defun jpj/projectile-open-root-in-cursor ()
  (interactive)
  (shell-command (format "code %s" (projectile-acquire-root))))

(define-key projectile-mode-map (kbd "C-c p F") 'jpj/projectile-open-root-in-finder)
(define-key projectile-mode-map (kbd "C-c p z") 'jpj/projectile-open-root-in-cursor)
...
(defun eglot-ensure-if-not-ssh ()
    (if (not (s-starts-with? "/ssh:july@tana" (buffer-file-name)))
        (eglot-ensure)))
...
(use-package biomejs-format
  :straight (:host github :repo "yadex205/emacs-biomejs-format" :branch "master")
  :config
  ...
  (add-hook 'jpj-js-mode-hook 'biomejs-format-mode)
...
** org-rich-yank: paste other buffer content into org-mode nicely
...
#+BEGIN_SRC emacs-lisp
(defun supernal-django-shell ()
  (interactive)
  (let ((default-directory "/Users/jeroen/code/supernal/foundation/src/foundation")
        (python-shell-interpreter "/Users/jeroen/.local/bin/poetry run python")
        (python-shell-interpreter-args "/Users/jeroen/code/supernal/foundation/src/foundation/manage.py shell_plus"))
    (run-python)))
#+END_SRC

#+RESULTS:
: supernal-django-shell
...
(defvar gptel-prompts-directory
  (expand-file-name "gptel-prompts" user-emacs-directory)
  "Directory containing GPTel prompt files.")

;; Helper to read prompt files
(defun gptel-load-prompt (filename)
  "Load prompt from FILENAME in `gptel-prompts-directory'."
  (let ((file (expand-file-name filename gptel-prompts-directory)))
    (if (file-exists-p file)
        (with-temp-buffer
          (insert-file-contents file)
          (buffer-string))
      (error "Prompt file %s not found" file))))

;; Create the prompts directory if it doesn't exist
(unless (file-exists-p gptel-prompts-directory)
  (make-directory gptel-prompts-directory t))

(use-package gptel
  :straight (:host github :repo "karthink/gptel" :branch "master")
  :config
  (setq gptel-max-tokens 2500
        gptel-default-mode 'org-mode
        )

  (setq gptel-directives
      `((default . ,(gptel-load-prompt "default.org"))
        (reason . ,(gptel-load-prompt "reason.org"))
        ;; (programming . ,(gptel-load-prompt "programming.org"))
        ;; (writing . ,(gptel-load-prompt "writing.org"))
        ;; (chat . ,(gptel-load-prompt "chat.org"))
        ))
  ...
  ;;         :stream t :key gptel-claude-3-api-key :models '("claude-opus-4-5-20251101")))

  ;; (setq gptel-gemini-backend
  ;;       (gptel-make-gemini "gemini"
  ;;         :key gptel-gemini-api-key :stream t :models '("gemini-2.0-flash-exp")))

  (setq gptel-vercel-backend
        (gptel-make-openai "vercel"
          :host "ai-gateway.vercel.sh"
          :endpoint "/v1/chat/completions"
          :stream t
          :key gptel-vercel-ai-api-key
          :models '("openai/gpt-5"
  ...
  (setq gptel-opencode-backend (gptel-make-openai "opencode-zen"
                        :host "opencode.ai"
                        :endpoint "/zen/v1/chat/completions"
                        :key opencode-zen-api-key
                        :models '("claude-opus-4-6" "gpt-5.1-codex") ; Add your preferred models
                        :stream t))


  (setq
   gptel-model 'claude-opus-4-6
   gptel-backend gptel-opencode-backend)
  (setq my/gptel-api-key-label "claude-private-mode")
  ;; (setq
  ;;  gptel-model "gemini-2.0-flash-exp"
  ;;  gptel-backend gptel-gemini-backend)
  ;; (setq my/gptel-api-key-label "gemini-private-mode")

  (cl-defun gptel-make-azure-llama
    (name &key curl-args models stream
          (protocol "https")
          (host (dns-azure-host))
          ...
          (key (dns-azure-api-key))
          request-params)
  "Register an Azure Llama backend for gptel with NAME.

Keyword arguments:
CURL-ARGS (optional) is a list of additional Curl arguments.
HOST is the Azure API host.
...
HEADER is for additional headers to send with each request.
KEY is the API key or a function that returns it.
REQUEST-PARAMS is a plist of additional HTTP request parameters."
  (declare (indent 1))
  (let ((backend (gptel--make-openai
                  ...
                  :header header
                  :key key
                  :models (gptel--process-models (or models '(llama-70b)))
                  :protocol protocol
                  :endpoint endpoint
                  :stream stream
  ...
  (defun new-chat ()
    "Start a new chat session with a unique buffer name."
    (interactive)
    (gptel (generate-new-buffer-name "*llm*") nil nil t)
    (auto-fill-mode 0)
    (visual-line-mode t))

  ;; Screen functions (keeping your buffer collection methods)
  (defun gpt-screen ()
        ...
        (let* ((buf (window-buffer win))
          ...
          (with-current-buffer buf
            (setq content (concat content
                                  "**** " buf-name "\n\n#+BEGIN_EXAMPLE\n"
                                  (buffer-string) "\n#+END_EXAMPLE\n\n")))))
      (setq content (concat content "*** now please answer the following question:\n\n"))
      (gptel (generate-new-buffer-name "*llm screen*") nil content t))
    (auto-fill-mode 0)
    (visual-line-mode t))

  (defun gpt-screen-visible ()
    "Take the contents of each visible part of buffers on screen."
    (interactive)
    (let ((content "*** Here is some code I am working on:\n\n"))
      (dolist (win (window-list))
        (let* ((buf (window-buffer win))
  ...
  (defun gpt-screen-current-visible ()
    "Process the visible part of the current buffer."
    (interactive)
...
(bind-key* "C-s-l" 'hydra-gptel/body)
#+END_SRC
...
(use-package aidermacs
  ...
  (setenv "ANTHROPIC_API_KEY" gptel-claude-3-api-key)
  :custom
...
#+BEGIN_SRC emacs-lisp
(defun my-sql-postgres-foundation ()
  (interactive)
  ...
  (setq sql-connection-alist
        '((foundation
           ...
           (port 5432)
           (ssl nil))))
  (sql-connect 'foundation))

(global-set-key (kbd "C-c s f") 'my-sql-postgres-foundation)
#+END_SRC

#+RESULTS:
: my-sql-postgres-foundation
...
(defun copy-pytest-failures ()
  "Copy text between the FAILURES and warnings summary markers in the current buffer to the kill ring."
  (interactive)
  (save-excursion
    (goto-char (point-min))
    ;; Find the starting marker and record its beginning position.
    (if (search-forward "====================================== FAILURES =======================================" nil t)
        (let ((failures-start (match-beginning 0)))
          ;; Now search for the ending marker from the beginning.
          (goto-char failures-start)
          (if (search-forward "================================== warnings summary ===================================" nil t)
              (let ((failures-end (match-beginning 0)))
                (kill-ring-save failures-start failures-end)
                (message "Copied pytest failures to kill ring."))
            (message "Ending marker not found.")))
      (message "Starting marker not found."))))
...
"
  ...
  ("t" copy-pytest-failures)
  ("m" mulrun-hydra)
  ("pp" cpath)
  ("pr" crpath)
  ("pm" crmod)
  ("pc" copy-rel-mpc-path)
)
...
(use-package visual-fill-column
  ...
  (setq visual-fill-column-extra-text-width '(4 . 0))
...
#+RESULTS:
| (lambda nil (setq-local visual-fill-column-width 120) (visual-line-mode 1) (visual-fill-column-mode 1)) | visual-line-mode | visual-fill-column-mode | my/on-gptel-mode |
